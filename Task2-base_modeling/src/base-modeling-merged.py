...
def run_base_modeling_loto(
    ...
    for i in range(num_repeats):
        ...
        for X, y in zip(X_all, y_all):
            ...
            # 特征选择
            k = min(60, X_train.shape[1])  # 取较小值
            if regression:
                selector = SelectKBest(mutual_info_regression, k=k)
                X_train = selector.fit_transform(X_train, y_train)
            else:
                selector = SelectKBest(mutual_info_classif, k=k)
                X_train = selector.fit_transform(X_train, y_train_category)
...
def run_base_modeling_random(
        data_root, output_root, 
        num_repeats=1, model_type="rf", task='binary', test_size=0.2,
        use_gridsearch=True, regression=True, seed=42
    ):
    set_seed(seed)
    # 加载数据
    ...
    X_all, y_all = load_data(data_root)
    C, B, N = X_all.shape
    X_all = X_all.reshape(C * B, N)
    y_all = y_all.reshape(C * B)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="task1",
        help="输入数据文件来源：official, task1, 或数据文件所在目录"
    )
    parser.add_argument(
        "--output_root",
        type=Path,
    ...
    # 准备输入数据目录data_root
    if args.data_root == "task1":
        data_root = project_root / "Task1-preprocess" / "data" / "task2" / "npz"
    elif args.data_root == "official":
        data_root = project_root / "data_preprocessed_python"
    else:
        data_root = Path(args.data_root)
    print("Data root:", data_root)

    # 准备输出目录output_root
    if args.data_root in ["task1", "official"]:
        data_dir = args.data_root + "-preprocess"
    else:
        data_dir = "others"
    task_dir = args.task + "-" + args.split
    model_dir = args.model_type+"-regression" if args.regression else args.model_type
    output_root = Path(args.output_root) / data_dir / task_dir / model_dir
    output_root.mkdir(parents=True, exist_ok=True)
    print("Output root:", output_root)

    if args.split == "loto":
        train_acc_all, test_acc_all, test_pred_all, test_real_all = run_base_modeling_loto(
            data_root = data_root,
        ...
        train_acc_all, test_acc_all, test_pred_all, test_real_all = run_base_modeling_random(
            data_root = data_root,
            output_root = args.output_root,
            num_repeats = args.num_repeats, 
            model_type = args.model_type,
    ...
    if args.task == "binary":
        category_labels = ["N", "P"]
    elif args.task == "threeclass":
        category_labels = ["N", "U", "P"]

    log_file = Path(output_root) / "results.txt"
    logging.basicConfig(
        level=logging.INFO,
        format='%(message)s',
        handlers=[
    ...
    fig_file = Path(output_root) / "confusion_matrix.png"
